...
from .utils.recommender.llm_recommender import generate_hf_recommendation
from .utils.deployment_plans import generate_deployment_plan_payload, validate_target
...
def _get_analysis(analysis_id):
    if not analysis_id:
        return None, Response({"error": "analysis_id is required"}, status=400)
...
def analyze_repo(request):
    """Request body: {"repo_url": "<github repo url>"}"""
...
def recommend_deployment_ml(request):
    """Request body: {"analysis_id": "<uuid>", "cloud": "aws|gcp|azure"}"""
    analysis_id = request.data.get("analysis_id")
    cloud = request.data.get("cloud")

    if not analysis_id or not cloud:
        return Response({"error": "analysis_id and cloud are required"}, status=400)
    ...
    llm_recommendation = generate_hf_recommendation(insights, features, cloud)
    resources = llm_recommendation.get("predicted_resources") or predict_resources(features)
    ...
    return Response({
        ...
        "os_recommendation": llm_recommendation.get("os_recommendation"),
        "deployment_steps": llm_recommendation.get("deployment_steps"),
        "configuration_files_needed": llm_recommendation.get("configuration_files_needed"),
    })


@api_view(["POST"])
def deployment_plan(request):
    """Request body: {"analysis_id": "<uuid>", "cloud": "aws|gcp|azure", "target": {...}}"""
    analysis_id = request.data.get("analysis_id")
    cloud = request.data.get("cloud")
    target = request.data.get("target")

    if not analysis_id or not cloud or not target:
        return Response({"error": "analysis_id, cloud and target are required"}, status=400)
    ...
    if plan_error:
        return Response({"error": plan_error}, status=400)
